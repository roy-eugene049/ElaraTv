  ...
  <div class="content-main-block mrg-t-40">
    <div class="admin-create-btn-block">
      @can('banneradd.create')
        <a href="{{route('banneradd.create')}}" class="btn btn-danger btn-md" title="{{ __('Create Post') }}"><i class="material-icons left">add</i> {{__('Create Post')}}</a>
      @endcan
      <!-- Delete Modal -->
            ...
            <div class="card-two card">
              ...
              <div class="dropdown card-dropdown">
                ...
                <div class="dropdown-menu pull-right" aria-labelledby="dropdownMenuButton-{{$item->id}}">
                  ...
                  @can('banneradd.edit')
                    <a class="dropdown-item" href="{{ route('blog.edit', $item->id)}}" title="{{ __('Edit') }}"><i class="material-icons">mode_edit</i> {{__('Edit')}}</a>
                  @endcan
                
                  @can('banneradd.delete')
                  
                    <a type="button" class="dropdown-item" data-toggle="modal" data-target="#deleteModal{{$item->id}}" title="{{ __('Delete') }}"><i class="material-icons">delete</i> {{__('Delete')}}</a>
              ...
              <div class="card-body">
                ...
                {{-- <div class="card-block">
                  ...
                  <div class="card-icons">
                    <ul>
                      <li>
                          <a href="{{url('account/blog/', $item->slug)}}" data-toggle="tooltip" data-original-title=" Page Preview" target="_blank" class="btn-default btn-floating" title="{{ __('Preview') }}"><i class="material-icons">desktop_mac</i></a>
                      </li>
                      @can('blog.edit')
                        <li>
                          <a href="{{route('blog.edit', $item->id)}}" class="btn-info btn-floating" title="{{ __('Edit') }}"><i class="material-icons">mode_edit</i></a>
                        </li>
                      @endcan
                      @can('blog.delete')
                        <li>
                          <a type="button" class="btn-danger btn-floating" data-toggle="modal" data-target="#deleteModal{{$item->id}}" title="{{ __('Delete') }}"><i class="material-icons">delete</i> </a>
                          <div id="deleteModal{{$item->id}}" class="delete-modal modal fade" role="dialog">
                            <div class="modal-dialog modal-sm">
                              <!-- Modal content-->
                              <div class="modal-content">
